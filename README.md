#### 一、[binlog 日志说明][10]
#### 二、[MySQL 数据文件说明和文件碎片整理][15]
#### 三、[MySQL 表分区，提高单表数据存储量和查询效率简单使用 ][14]
#### 四、[Centos Oracle-MySQL-8.0单节点搭建][1]（不推荐使用）
#### 五、[Centos Percona-Server-5.7.26单节点搭建][3]（推荐生产使用）
#### 六、[Centos Percona-Server-5.7.26镜像全量强一致性集群搭建][4]（推荐生产使用）
#### 七、[Centos Percona-Server-5.7.26镜像全量弱一致性集群搭建][12]（推荐生产使用）
#### 八、[Centos Percona-Server-5.7.26集群强一致性数据同步流程以及普通MySQL集群弱一致性数据同步简要说明][11]
#### 九、[数据迁移工具 PT-Archiver 简单使用][13]
#### 十、[MyCat 主键取模分片搭建（注意：不要使用 MyCat 的UUID生成策略，原因 MyCat 生成的UUID大多都是偶数）][5]
#### 十一、[MyCat 根据某个字段的值分片搭建][6]（推荐生产使用）
#### 十二、[MyCat 父子表（某一条数据在哪个分片，其关联数据就在哪个分片）简单使用][7]
#### 十三、[MySQL 单表性能测试 Sysbench 简单使用][8]
#### 十四、[MySQL 单表和多表关联压力测试 Tpcc-MySQL 简单使用][9]
#### 十五、[Explain执行计划说明][2]
#### 十六、常用操作简单使用
```bash
...
#### 二十、数据查询导入（注意：如果文件太大，在Linux系统下可以使用 split 命令切分文件）
```bash
...
```
##### 21.4 删除[vi /etc/my.cnf]配置文件里面的 skip-grant-tables（跳过用户名密码验证）
##### 21.5 重启mysql服务
```bash
$ service mysqld restart                                       # 重启服务
```
#### 二二、SQL优化（注意：insert 语句后面加 IGNORE 关键字，如果插入数据违反了唯一约束（比如主键或唯一索引），不会报错，会返回受影响行数  0，建议生产使用，比如要查询用户手机是否存在，我直接插入数据就行了，返回0就说明用户手机已存在。插入语句示例：insert ignore into user(name)values(?)）
```bash
