...
```
#### 二、父子表简单说明（注意：不推荐使用，大并发场景插入速度慢，因为要先查询到父表记录在哪个分片上）
```bash
1，比如user表分片，用户信息在哪个分片上，它的订单数据就全部在哪个分片上（注意：订单表可以看作是子表，但是不能再分片）
```
#### 三、下载 Mycat-Server 安装包
```bash
$ wget -P /home/tools http://dl.mycat.io/1.6.7.1/Mycat-server-1.6.7.1-release-20190627191042-linux.tar.gz
```

#### 四、安装Mycat-Server（注意：Mycat-Server实际是模拟MySQL节点）
```bash
$ cd /home/tools
# 解压到上层目录
...
#### 五、修改[vi /home/mycat/conf/server.xml]配置虚拟用户名，密码，逻辑库。供客户端使用（注意：先删除所有<user>标签，然后在文件末尾添加）
```bash
...
#### 六、修改[vi /home/mycat/conf/schema.xml]配置连接，读写分离，负载均衡，数据表映射（注意：先删除原有的配置信息，然后在文件末尾添加）
```bash
<!-- 配置数据库 test_test -->
<schema name="test_test" checkSQLschema="false" sqlMaxLimit="100">
    ...
    <table name="m_user" dataNode="dn1,dn2" rule="mod-long">
         <!-- 父子表配置（注意：可以包含多个子表）-->
         <!-- 配置子表 m_order（primaryKey子表主键字段名称，parentKey父表主键字段名称，joinKey子表关联父表字段名称）-->
        <childTable name="m_order" primaryKey="id" parentKey="id" joinKey="user_id"/>
    </table>
</schema>
...
#### 七、修改[vi /home/mycat/conf/rule.xml]配置切片算法相关信息（注意：我们上面使用了 mod-long（取模切片）算法，所以我们只需要修改切片数量即可（配置项已经存在，只需要修改值即可））
```bash
